...
def simulate_Data(data_folder_file_path):
    ...
    physical_data_raw               = pd.read_csv(data_folder_file_path+"filtered_data.csv", index_col=False, header=None)
    physical_data_raw               = np.array(physical_data_raw)
    for p in range(1,int(physical_data_raw.shape[0]/100)):
        physical_data               = physical_data_raw[:(p+1)*100]
        physical_data               = physical_data[-101:]
